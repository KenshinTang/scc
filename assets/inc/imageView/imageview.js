    ...
    */
    ...
    Vimg.prototype.useDataToImage = function () {
        ...
        if (s.image) {
            ...
            // if(s.position.x&&s.position.x>0){
        	// 	s.image.css({
	        //         zIndex: 2,
	        //         top: s.top + 'px',
	        //         left: s.left + 'px',
    ...
    var _Interaction = {
        ...
        */
        ...
        gesturestart: function () {
            console.log('当有两根或多根手指放到屏幕上的时候触发');
            //当前页的图片对象
            var vimg = ImageView.vImageList[ImageView.page - 1];
        ...
        gesturechange: function (touch1, touch2) {
            console.log('当有两根或多根手指在屏幕上，并且有手指移动的时候触发');
            ...
            if (isType(selector, 'array')) {
                ...
                for (var i = 0; i < currentTarget.length; i++) {
                    ...
                    if (item.nodeName.toLowerCase() === 'img') {
                        //img标签
                        var big = item.getAttribute('big');
                        list.push(new Vimg({ src: (big ? big : item.src), target: item }));
                    } else {
                        //背景
