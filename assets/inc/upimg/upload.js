...
imgUploader.prototype.addImgItem = function (src, del) {
    ...
    ibox.className = "box";
    ibox.innerHTML = '<img id="imgDel' + (this.imgList.length - 1) + '" onerror="this.src=\'' + this._errorSrc + '\';this.onerror=null" class="upimg" src="' + Comm.OSS.getImgUrl(src, 's', this._errorSrc) + '">' + (del ? '<div class="upimg-del" onclick="imgUploader[\'' + this.subbutton + '\'].del(' + (this.imgList.length - 1) + ');"></div>' : '');
...
imgUploader.prototype.del = function (i) {
    ...
    Comm.confirm('您确定要删除该图片？', function (a) {
        if (parseInt(a) > 0) {
            ...
            var nowNum = me.maximg - me.imgList.length;
            me.imgList.splice(i - nowNum, 1);
            me.delimgs(i, 1);
...
function get_signature() {
    if (up_expire < Date.parse(new Date()) / 1000 + 3 || true) {
        ...
        serverUrl = config.root + "common/oss/getImgPolicy?_token=" + Comm.db("_token") + "&_device=" + _d_;
        xmlhttp.open("GET", serverUrl, false);
        xmlhttp.send(null);
        up_token = JSON.parse(xmlhttp.responseText);
        if (up_token.code == 110) {
            Comm.alert("登录信息已过期，请重新登录" + (config.SLTest ? '(括号里面是测试的正式发包不会有upload.js)' : ''), function () {
                AJAX.clear();
                Comm.go('login.html')
            })
            return
